...
parser.add_argument('--model_name', type=str, help='model name could be either unet, unetpp or combined')


class DataLoaderSegmentation(torch.utils.data.Dataset):
    def __init__(self, folder_path, class_rgb_values=None, img_preprocessing=None, mask_preprocessing=None):
...
def main(batch_size=16, epochs=2, model_name='unet'):
    ...
    valid_loader = DataLoader(valid_dataset, batch_size=batch_size, shuffle=False, num_workers=8, pin_memory=True, drop_last=True)
    ...
    for epoch in range(1, epochs+1):
        ...
        for batch in trainloader:
            ...
            if i%100 == 0:
                ...
                with open(f"train_losses/unet_bs_{batch_size}_losses_{epoch}.txt", 'w') as f:
        ...
        for batch in valid_loader:
            with torch.no_grad():
                ...
                f'Validation:\tepoch: {epoch}/{epochs}\tloss: {"{:.4f}".format(val_mean)}'
            )
        total_val.append(val_mean)
        with open(f"train_losses/unet_val_bs_{batch_size}_losses_{epoch}.txt", 'w') as f:
